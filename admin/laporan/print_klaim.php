...
// Ambil data klaim seluruh perusahaan bulan ini
$stmtKlaim = $conn->prepare("
    SELECT km.no_klaim, km.tanggal_masuk, km.total_tagihan_faskes as total_tagihan, km.status_klaim, km.total_dibayarkan_asuransi,
           p.no_polis, pp.nama_lengkap as nama_nasabah, a.nama_agen, f.nama_faskes
    FROM klaim_medis km
    ...
    WHERE DATE_FORMAT(km.tanggal_masuk, '%Y-%m') = ?
    ORDER BY km.tanggal_masuk DESC
");
$stmtKlaim->execute([$bulan]);
...
foreach ($klaim_list as $klm) {
    ...
    if ($klm['status_klaim'] === 'Approved') {
        $totalKlaimDisetujui += $klm['total_dibayarkan_asuransi'];
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Laporan Klaim - <?php echo htmlspecialchars($namaBulan); ?></title>
    ...
    <div class="paper">
        ...
        <div class="paper-head">
            ...
            <div style="text-align: right;">
                <div style="font-size: 14px; font-weight: 800; color: var(--ink); text-transform: uppercase;">LAPORAN KLAIM NASIONAL</div>
                    ...
                    <?php if (count($klaim_list) > 0): ?>
                        <?php foreach($klaim_list as $i => $row): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($row['nama_faskes']); ?></td>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="7" class="c" style="padding: 15px;">Tidak ada klaim medis yang diajukan pada bulan ini.</td></tr>
                    <?php endif; ?>
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="5" class="r" style="font-size: 9px; font-weight: 800; color: var(--ink); padding-right: 15px;">TOTAL DIAJUKAN</td>
                        <td class="r" style="font-weight: 800; color: var(--ink);">Rp <?php echo number_format($totalKlaimDiajukan, 0, ',', '.'); ?></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td colspan="5" class="r" style="font-size: 9px; font-weight: 800; color: var(--success); padding-right: 15px;">TOTAL DISETUJUI & DIBAYARKAN</td>
        ...
        <div class="paper-foot">
            <div class="foot-note">
                ...
                Nilai klaim disetujui merepresentasikan total pengeluaran beban asuransi perusahaan pada bulan terkait.
            </div>
            <div class="sign-col">
                <div style="font-size:8px;color:var(--ink-3);margin-bottom:2px;">Disahkan Oleh,</div>
                <div style="font-size:8px;color:var(--ink-2);font-weight:600;">Direktur Klaim & Risiko</div>
