...
$stmtNasabah = $conn->prepare("
    SELECT nik, nama_lengkap, alamat, no_telepon, created_at
    FROM pemegang_polis
    WHERE id_agen = ? AND DATE_FORMAT(created_at, '%Y-%m') = ?
    ORDER BY created_at DESC
");
$stmtNasabah->execute([$id_agen, $bulan]);
$nasabah_baru = $stmtNasabah->fetchAll(PDO::FETCH_ASSOC);

// Ambil data polis baru bulan ini
$stmtPolis = $conn->prepare("
    SELECT p.no_polis, p.tanggal_terbit, pr.nama_produk, pp.nama_lengkap
    FROM polis p
    JOIN pemegang_polis pp ON p.id_pemegang = pp.id_pemegang
    JOIN produk_asuransi pr ON p.id_produk = pr.id_produk
    WHERE p.id_agen = ? AND DATE_FORMAT(p.tanggal_terbit, '%Y-%m') = ?
    ORDER BY p.tanggal_terbit DESC
");
$stmtPolis->execute([$id_agen, $bulan]);
    ...
    <div class="paper">
            ...
            <table>
                ...
                <tbody>
                    <?php if (count($polis_baru) > 0): ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="5" class="c" style="padding: 15px;">Tidak ada polis baru yang diterbitkan pada bulan ini.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
        ...
        <div class="paper-foot">
            <div class="foot-note">
                ...
                Digunakan untuk evaluasi kinerja keagenan pada periode terkait.
            </div>
            <div class="sign-col">
