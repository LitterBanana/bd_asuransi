...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id_agen = $agent['id_agen'];

    // Cari tagihan tertunggak (paling lama) untuk polis ini
    $stmtTagihan = $conn->prepare("
        SELECT t.no_tagihan, t.jumlah_tagihan
        FROM tagihan_premi t
        JOIN polis p ON t.no_polis = p.no_polis
        LEFT JOIN pemegang_polis pp ON p.id_pemegang = pp.id_pemegang
        WHERE p.no_polis = ? AND (p.id_agen = ? OR pp.id_agen = ?) AND t.status_tagihan IN ('Unpaid', 'Overdue')
        ORDER BY t.jatuh_tempo ASC
        LIMIT 1
    ");
    $stmtTagihan->execute([$no_polis, $id_agen, $id_agen]);
    $tagihan = $stmtTagihan->fetch();

    if (!$tagihan) {
        header("Location: index.php?error=" . urlencode("Tidak ada tagihan tertunggak (Unpaid/Overdue) untuk polis ini, atau polis bukan milik Anda."));
        exit;
    }
